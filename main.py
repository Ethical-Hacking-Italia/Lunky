try:
    ...
    def hackez_console():
        ...
        time.sleep(1)
        console.print("\n(pls) type 'credits' or 'info' for more information.", style="green")
        while True:
            ...
            if cmd.lower() == 'help':
                ...
                if command == '' or command == 'port-scanner':
                    console.print("ERROR: type port-scanner -h for some help", style="red")
                elif command == '-h':
                    print("--------[ OPTIONS ]--------\n-i [ip]\n-out [file.extension]")
                    ...
                    console.print("ERROR: invalid option. Type port-scanner -h for some help", style="red")
    ...
    try:
        version = get("https://version.hackingtool.repl.co/version.txt").text
        ...
        console.print(f"\n[!] ERROR: failed to get the current version (no internet connection)", style="red")
        exit()
    try:
        informations = get("https://version.hackingtool.repl.co/informations.txt").text
        progress_bar(80, 100, "blue")
    except Exception:
        time.sleep(0.5)
        progress_bar(0, 100, "red")
        console.print(f"\n[!] ERROR: failed to get the informations (no internet connection)", style="red")
        exit()
    ...
    console.print(icon)
    if old == True:
        console.print(f"\nthis version of the tool is old, update it by downloading the new version on GitHub\n          this version: {this_version}   new version: {version}", style="red")
    tool()
